<template>
    ...
    <TransitionGroup name="lists" class="tasks" tag="ul">
        <li class="border-b border-b-gray-200 pb-4 mb-3 shadow-xs pl-4" v-for="task in todayTasks" :key="task.id">
...
const { todayTasks } = storeToRefs(todoStore);
...
// const todayTasks = tasks.value.filter(el => new Date(el.due_date.seconds * 1000).toDateString() == new Date().toDateString());
